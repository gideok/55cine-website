...
function loadListMeta() {
  ...
  if (!Array.isArray(window.GV_MOMENT_DATA)) {
    throw new Error("GV_MOMENT_DATA not found");
  }
  return window.GV_MOMENT_DATA;
}

function extractDetailHtml(html, numericId) {
  const id = "gm" + String(numericId).padStart(3, "0");
  ...
  if (!coverImage) {
    coverImage = `images/magazine/gv-moment/gv_moment_thumb_${String(numericId).padStart(2, "0")}.png`;
  }
...
function main() {
  ...
  for (let n = 1; n <= 24; n++) {
    const file = path.join(GV_DIR, `gv-moment-detail-${String(n).padStart(2, "0")}.html`);
