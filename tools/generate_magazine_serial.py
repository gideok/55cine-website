"""
매거진 삼삼오오 > 연재 카테고리에서 샘플 24건 수집, 썸네일·상세 HTML·data/*.js 생성.
실행: 프로젝트 루트에서 python tools/generate_magazine_serial.py
"""
...
# 장르 불문! 대구독립영화
CATEGORY = (
    "/category/%EC%9E%A5%EB%A5%B4%20"
    "%EB%B6%88%EB%AC%B8%21%20"
    "%EB%8C%80%EA%B5%AC%EB%8F%85%EB%A6%BD%EC%98%81%ED%99%94"
)

IMG_DIR = ROOT / "images/magazine/serial"
DETAIL_DIR = ROOT / "magazine/serial"
DATA_PATH = ROOT / "data/magazine-serial-data.js"
...
def strip_inline_font_family_from_style_attrs(fragment: str) -> str:
    ...
    return re.sub(r'\sstyle="([^"]*)"', scrub_attr, fragment)
    ...
    for page in range(1, 40):
        ...
        for post in posts:
            ...
            if href in seen:
                continue
            seen.add(href)
            img_el = post.select_one("img[src]")
...
<body>
  ...
  <main>
    <article class="detail-wrap" aria-labelledby="article-title">
      <p class="detail-kicker">매거진 삼삼오오 · 연재</p>
      <h1 id="article-title" class="detail-title">{html.escape(title)}</h1>
      <p class="detail-meta">{html.escape(date if date else "연재 · 장르 불문! 대구독립영화")}</p>
      <figure class="detail-cover"><img src="../../{thumb_rel}" alt="{html.escape(title)} 썸네일" /></figure>
      <section class="detail-body">{body_html}</section>
      <nav class="detail-nav magazine-pn" aria-label="연재 상세 이동">
        <a class="detail-back" href="../../magazine-serial.html">← 연재 목록으로</a>
...
def main():
    ...
    for idx, item in enumerate(items, start=1):
        ...
        thumb_rel = f"images/magazine/serial/serial_thumb_{idx:02d}.png"
        thumb_abs = ROOT / thumb_rel
        save_thumbnail(thumb_url, (239, 234, 220), thumb_abs)
        ...
        enriched.append({
            ...
            "date": date,
            "thumbnail": thumb_rel,
            "detailUrl": f"magazine/serial/magazine-serial-detail-{idx:02d}.html",
    ...
    for idx, item in enumerate(enriched, start=1):
        if idx > 1:
            prev = enriched[idx - 2]
            prev_card = (
                f'<a class="pn-card" href="magazine-serial-detail-{idx-1:02d}.html">'
        ...
        (DETAIL_DIR / f"magazine-serial-detail-{idx:02d}.html").write_text(html_text, encoding="utf-8")

    payload = [{k: v for k, v in it.items() if k != "bodyHtml"} for it in enriched]
    DATA_PATH.write_text(
        "window.MAGAZINE_SERIAL_DATA = " + json.dumps(payload, ensure_ascii=False, indent=2) + ";\n",
        encoding="utf-8",
    )
